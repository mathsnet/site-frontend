<template>
  <div>
    <v-dialog v-model="openDialog" :width="width" persistent>
      <v-card>
        <v-card-title class="primary--text">Add New Instructor</v-card-title>
        <v-card-text>
          <v-form
            ref="newInstructorForm"
            v-model="valid"
            @submit.prevent="addNewInstructor"
          >
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  ...
                  label="First Name"
                  hint="Enter the Instructor Name"
...
export default {
  ...
  methods: {
    ...
    async addNewInstructor() {
      if (!this.$refs.newInstructorForm.validate()) {
        this.$store.dispatch(
      ...
      try {
        const { data } = await this.$axios.post(
          CONSTANTS.ROUTES.ADMIN.ADD_INSTRUCTOR,
          {
